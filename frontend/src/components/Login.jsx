...
function Login() {
  ...
  const handlePasswordInput = (field, setter, label) => (e) => {
      ...
      <div className=" text-black font-semibold text-2xl px-20 py-12 flex justify-between">
        ...
        <div className="flex items-center justify-between">
          <a className="flex items-center mr-8 font-semibold" href="">
            English <span className="text-4xl ml-2 font-bold"><MdExpandMore /></span>
          </a>
          <Link to="/login">
            <button className="flex 0 text-black py-3 px-6 border-4 border-black rounded-full font-semibold">
              <span className="text-2xl mr-4 mt-1"><LuArrowRightToLine /></span>Login/Signup
            </button>
          </Link>
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="relative">
              <input
                ...
                value={password}
                onChange={handlePasswordInput("password", setPassword, "Password")}
                className="w-full border rounded p-2 mb-1"
                required
              />
              <button
              ...
              <div className="bg-white p-4 rounded border mb-4">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setSingleFieldError("forgotEmail", value ? validateEmail(value) : "");
                    setErrorMessage("");
                  }}
                  className="w-full border rounded p-2 mb-1"
                />
                {fieldErrors.forgotEmail ? (
                  <p className="text-red-500 text-xs mb-3">{fieldErrors.forgotEmail}</p>
                ) : (
                  <div className="mb-3"></div>
                )}
                ...
                <div className="relative">
                  <input
                    ...
                    onChange={handlePasswordInput("forgotNewPassword", setForgotNewPassword, "New password")}
                    className="w-full border rounded p-2 mb-1"
                  />
                  <button
                    type="button"
                ...
                {fieldErrors.forgotNewPassword ? (
                  <p className="text-red-500 text-xs mb-3">{fieldErrors.forgotNewPassword}</p>
                ) : (
                  <div className="mb-3"></div>
                )}
                ...
                <div className="relative">
                  <input
                    ...
                    onChange={handlePasswordInput("forgotConfirmPassword", setForgotConfirmPassword, "Confirm new password")}
                    className="w-full border rounded p-2 mb-1"
                  />
                </div>
                {fieldErrors.forgotConfirmPassword ? (
                  <p className="text-red-500 text-xs mb-3">{fieldErrors.forgotConfirmPassword}</p>
                ) : (
                  <div className="mb-3"></div>
                )}
                    ...
                    onClick={async () => {
                      ...
                      try {
                        const forgotErrors = {
                          ...
                          forgotNewPassword: validatePassword(forgotNewPassword, "New password"),
                          forgotConfirmPassword: validatePassword(forgotConfirmPassword, "Confirm new password"),
                        };

                        if (forgotNewPassword !== forgotConfirmPassword) {
                          forgotErrors.forgotConfirmPassword = "Passwords do not match.";
                        }

                        setFieldErrors((prev) => ({ ...prev, ...forgotErrors }));

                        const forgotError = Object.values(forgotErrors).find(Boolean);
                        if (forgotError) {
                          setErrorMessage(forgotError);
                          return;
                        }
                        ...
                        if (err.response && err.response.data) {
                          ...
                          const message = data.message || (Array.isArray(data.errors) ? data.errors.join(" ") : "Password reset failed");
                          setErrorMessage(message);
                        } else {
        ...
        <div className="bg-blue-950 h-lvh text-white px-40 py-14 flex justify-between">
          ...
          <div>
            ...
            <div>
              ...
              <p className="underline text-2xl text-light">Learn Mathametics Online</p>
